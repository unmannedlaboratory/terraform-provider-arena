...
page_title: "arena_agent Resource - terraform-provider-arena"
subcategory: ""
description: |-
  Use this resource to associate an agent.
---

# arena_agent (Resource)
...
- `shutdown_script` (String) A script to run before the agent is stopped. The script should exit when it is done to signal that the workspace can be stopped. This option is an alias for defining a "arena_script" resource with "run_on_stop" set to true.
...
- `startup_script` (String) A script to run after the agent starts. The script should exit when it is done to signal that the agent is ready. This option is an alias for defining a "arena_script" resource with "run_on_start" set to true.
