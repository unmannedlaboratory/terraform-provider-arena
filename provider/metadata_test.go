	...
	resource.Test(t, resource.TestCase{
		...
		Steps: []resource.TestStep{{
			Config: `
				...
				resource "arena_metadata" "agent" {
					resource_id = arena_agent.dev.id
					hide = true
			...
			Check: func(state *terraform.State) error {
				...
				agent := state.Modules[0].Resources["arena_agent.dev"]
				require.NotNil(t, agent)
				metadata := state.Modules[0].Resources["arena_metadata.agent"]
